...
func ControllerCreate(config *GlobalConfig) (*Controller, error) {
	...
	// Modem block
	mdm := modem.GsmModemCreate(config.ModemPort, config.Os, 9600, config.MyPhoneNumber)
	err = mdm.Open()
	...
	httpBlock.web, err = httpServer.HttpServerCreate(config.HttpAddress, httpBlock.answerChan, httpBlock.queryChan)
